...
public:
    ...
    void initializeWorld()
    {
        ...
        int ex = rand() % SIZE;
        int ey = rand() % SIZE;
        while (world[ex][ey] != ' ' || (ex == px && ey == py))      // check
        {
            ex = rand() % SIZE;
            ey = rand() % SIZE;
        }
    ...
    void movePlayerLevel(char direction, int& level)
    {
        ...
        if (world[new_x][new_y] != ' ')
        {
            ...
            if (world[new_x][new_y] == 'R')
            {
                ...
                arr[re_size1-1] = { "Found a Relic and a random Item" };
resizeArray(arr, re_size2, re_size1, re_size1+1);
                system("pause");
            }
            else if (world[new_x][new_y] == 'F')
            {
                ...
                arr[re_size1-1] = { "Found a Fountain and a random Item and gained 1 HP" };
resizeArray(arr, re_size2, re_size1, re_size1+1);
                system("pause");
            }
            else if (world[new_x][new_y] == 'X')
            {
                ...
                arr[re_size1-1] = { "Stepped on a " + trapType + "!" };
                resizeArray(arr, re_size2, re_size1, re_size1+1);
        ...
        if (new_x == enemy.getX() && new_y == enemy.getY())
        {
            ...
            player.setLife(player.getLife() - 1);
            arr[re_size1-1] = { " Got caught by the enemy!!" };
resizeArray(arr, re_size2, re_size1, re_size1+1);
            system("pause");
            getchar();
        }
...
void resizeArray(std::string*& arr, int& size, int& re_size1, int newSize)
{
